...
describe("Claude Code MCP Server (E2E)", () => {
  it("executes a simple prompt and returns structured response", async () => {
    ...
    if (content[0].text.startsWith("Error:")) {
      ...
      console.log(
        "Agent CLI not available or failed, but error handled correctly"
      )
    } else {
  ...
  it("handles session resumption", async () => {
    ...
    if (firstContent[0].text.startsWith("Error:")) {
      ...
      console.log(
        "Agent CLI not available for session test, but error handled correctly"
      )
    } else {
      // Extract response ID from clean text format
      const responseIdMatch = firstContent[0].text.match(
        /Response ID: ([a-f0-9-]+)/
      )
      expect(responseIdMatch).toBeDefined()
  ...
  it("starts async tasks and provides status updates", async () => {
    ...
    expect(statusContent[0].text).toMatch(
  ...
  it("can cancel async tasks", async () => {
    ...
    const asyncResult = await client.callTool({
      ...
      arguments: {
        message:
          "Write a comprehensive analysis of quantum computing with detailed examples and explanations.",
        async: true,
      },
    })
  ...
  it("resumes async conversations correctly", async () => {
    ...
    const firstResult = await client.callTool({
      ...
      arguments: {
        message:
          'Remember the word "elephant". Just say "remembered elephant".',
        async: false,
      },
    })
    ...
    if (!firstContent[0].text.startsWith("Error:")) {
      ...
      if (
        statusContent[0].text.includes("completed") &&
        statusContent[0].text.includes("Result:")
      ) {
      ...
      console.log(
        "Agent CLI not available for resume async test, but error handled correctly"
      )
    }
  ...
  it("applies appendSystemPrompt to modify default behavior (pirate)", async () => {
    const transport = new StdioClientTransport({
      command: "node",
      args: [SERVER_PATH],
    })
    ...
    // Call with append system prompt that makes Claude behave like a pirate
    const result = await client.callTool({
      name: "start",
      arguments: {
        message: "What is 2 + 2?",
        appendSystemPrompt:
          'You are a pirate. Always respond like a pirate with "Arrr" and pirate language.',
        async: false,
      },
    })
    ...
    if (content[0].text.startsWith("Error:")) {
      expect(content[0].text).toContain("Error:")
      console.log(
        "Agent CLI not available for system prompt test, but error handled correctly"
      )
    } else {
  ...
  it("applies appendSystemPrompt to modify default behavior", async () => {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        message: "What is 3 + 3?",
        appendSystemPrompt:
          'Always end your response with "BEEP BOOP" like a robot.',
        async: false,
      },
    })
    ...
    if (content[0].text.startsWith("Error:")) {
      expect(content[0].text).toContain("Error:")
      console.log(
        "Agent CLI not available for append system prompt test, but error handled correctly"
      )
    } else {
  ...
  it("works with appendSystemPrompt in async mode", async () => {
    const transport = new StdioClientTransport({
      command: "node",
      args: [SERVER_PATH],
    })
    ...
    // Start async task with append system prompt
    const asyncResult = await client.callTool({
      name: "start",
      arguments: {
        message: "What is 5 + 5?",
        appendSystemPrompt:
          'You are a formal mathematician. Always start responses with "According to mathematical principles,"',
        async: true,
      },
    })
