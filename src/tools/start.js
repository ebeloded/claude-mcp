...
export const startTool = {
  ...
  schema: {
    ...
    async: z
      ...
      .default(true)
      .describe(
        "Whether to execute the task asynchronously. When true (default), returns a task ID immediately and runs in background. When false, returns result synchronously."
      ),
  },
  handler:
    (claudeService, taskService) =>
    async ({ message, workingDirectory, appendSystemPrompt, async }) => {
      try {
        ...
        if (async) {
          ...
          return createSuccessResponse(
            `Task started successfully. Use status with task ID: ${taskId}`
          )
        } else {
          // Sync execution - return result immediately
          const result = await claudeService.executeSync(
            message,
            undefined,
            workingDirectory,
            options
